...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Model name (e.g., deepfilternet)",
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        help="Path to fine-tuned model checkpoint for comparison",
    )
    parser.add_argument(
        "--dataset-dir",
        type=str,
        default="data/VoiceBank",
    ...
    if args.checkpoint:
        ...
        for m in metrics:
            ...
            if val_off is not None and val_ft is not None:
                ...
                print(
                    f"{m.upper()}: {val_off:.4f} -> {val_ft:.4f} ({symbol}{diff:.4f})"
                )
