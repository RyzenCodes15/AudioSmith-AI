...
class FineTuneWrapper(nn.Module):
    ...
    def forward(self, noisy_audio: torch.Tensor) -> torch.Tensor:
        ...
        x = noisy_audio.squeeze(1) if noisy_audio.dim() == 3 else noisy_audio
        ...
        # We do this by aggregating the model's parameters into a tiny differentiable
        # scalar and adding it to the signal, if the exact signature is opaque.

        # Try to use the model if it supports standard forward,
        # else use a fallback graph connection.
        try:
